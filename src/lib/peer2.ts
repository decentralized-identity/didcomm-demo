...
const reverseCommonStringAbbreviations: Record<string, string> = Object.entries(commonStringAbbreviations)
...
export default class DIDPeer {
  ...
  static keyToMultibase(key: Uint8Array, prefix: multicodec.CodecName): string {
    return multibase.encode('base58btc', multicodec.addPrefix(prefix, key)).toString();
  }
  ...
  static base64UrlDecode(input: string): string {
    ...
    switch (base64.length % 4) {
      case 2: base64 += "=="; break; // 2 padding chars
      case 3: base64 += "="; break;  // 1 padding char
    }
  ...
  // Generate a DID
  static generate(signingKeys: Uint8Array[], encryptionKeys: Uint8Array[], serviceBlock: object): string {
    const didPrefix = 'did:peer:2';

    // Encode keys
    const encodedSigningKeys = signingKeys.map(key => {
      const encoded = multibase.encode('base58btc', multicodec.addPrefix('ed25519-pub', key));
      return '.' + purposeCodeList['Verification'] + encoded.toString();
    });

    const encodedEncryptionKeys = encryptionKeys.map(key => {
      const encoded = multibase.encode('base58btc', multicodec.addPrefix('x25519-pub', key));
    ...
    return didPrefix + encodedSigningKeys.join('') + encodedEncryptionKeys.join('') + finalService;
  }
  ...
  static resolve(did: string) {
    ...
    elements.forEach(element => {
      ...
      switch (purposeCode) {
        ...
        doc.authentication.push({ id: `#${DIDPeer.keyToIdent(decodedSigningKey, "ed25519-pub")}`, type: 'Ed25519VerificationKey2018', publicKeyMultibase: DIDPeer.keyToMultibase(decodedSigningKey, "ed25519-pub")});
        break;
        ...
        doc.keyAgreement.push({ id: `#${DIDPeer.keyToIdent(decodedEncryptionKey, "x25519-pub")}`, type: 'X25519KeyAgreementKey2019', publicKeyBase58: DIDPeer.keyToMultibase(decodedEncryptionKey, "x25519-pub")});
        break;

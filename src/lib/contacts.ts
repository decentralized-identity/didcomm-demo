...
export class LocalStorageContactService implements ContactService {
  ...
  getContacts(): Contact[] {
    const contactsString = localStorage.getItem(this.profilePrefix + LocalStorageContactService.CONTACTS_KEY);
  ...
  addContact(contact: Partial<Contact>): void {
    ...
    localStorage.setItem(this.profilePrefix + LocalStorageContactService.CONTACTS_KEY, JSON.stringify(contacts));
  }
  ...
  saveMessageHistory(contactId: number, messages: Message[]): void {
    localStorage.setItem(this.profilePrefix + LocalStorageContactService.MESSAGE_HISTORY_KEY + contactId, JSON.stringify(messages));
  }
}

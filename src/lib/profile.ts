...
export class LocalStorageProfileService extends ProfileService {
  ...
  setActiveProfile(profileId: string): void {
    localStorage.setItem(
      LocalStorageProfileService.ACTIVE_PROFILE_ID_KEY,
      profileId
    )
  }
  ...
  getActiveProfile(): Profile {
    ...
    const profileData = localStorage.getItem(
      LocalStorageProfileService.PROFILE_PREFIX + activeProfileId
    )
    if (!profileData) {
  ...
  saveProfile(profile: Profile): void {
    ...
    localStorage.setItem(
      LocalStorageProfileService.PROFILE_PREFIX + profile.id,
      serializedProfile
    )
  }
  ...
  deleteProfile(profileId: string): void {
    localStorage.removeItem(
      LocalStorageProfileService.PROFILE_PREFIX + profileId
    )
  }
  ...
  getProfileIds(): string[] {
    ...
    for (let i = 0; i < localStorage.length; i++) {
      ...
      if (key?.startsWith(LocalStorageProfileService.PROFILE_PREFIX)) {
        const profileId = key.replace(
          LocalStorageProfileService.PROFILE_PREFIX,
          ""
        )

    ...
    return m(".level", [
      m(".level-left", [
        m(".level-item", [
          m("div.select", [
            m("select", {
              onchange: (e: Event) => vnode.attrs.setGroup((e.target as HTMLSelectElement).value as LogGroups)
            }, [
              Object.values(LogGroups).map(group => m("option", { value: group }, group))
            ])
          ])
        ])
      ]),
...
class ConsoleComponent implements m.ClassComponent<ConsoleAttributes> {
  ...
  oncreate(vnode: m.VnodeDOM<ConsoleAttributes>) {
    ...
    setInterval(() => {
      this.appendLog({ message: `Log at ${new Date().toISOString()}`, group: LogGroups.Everything });
